...
def generate_instagram_data(tenant_slug: str, config: Any, days: int = 30) -> Dict[str, List[dict]]:
    """
    Generates consistent Instagram Ads data with relational integrity.
    Schema matches dlt verified source: campaigns, ad_sets, ads, facebook_insights.
    (Cloned from Facebook Ads to ensure identical schema for mapping registry testing)
    """
    ...
    campaign_ids = [f"{tenant_slug}_ig_camp_{i}" for i in range(n_campaigns)]
    campaigns_df = pl.DataFrame({
        "id": campaign_ids,
        "name": [f"IG | {tenant_slug.upper()} | {fake.catch_phrase()} | {fake.year()}" for _ in range(n_campaigns)],
    ...
    # --- 2. Ad Sets ---
    n_adsets = 15
    adset_ids = [f"{tenant_slug}_ig_adset_{i}" for i in range(n_adsets)]
    
    adsets_df = pl.DataFrame({
        "id": adset_ids,
        "campaign_id": np.random.choice(campaign_ids, n_adsets), # Link to Campaign
        "name": [f"IG Target: {fake.job()}" for _ in range(n_adsets)],
        "status": "ACTIVE",
    ...
    # --- 3. Ads ---
    n_ads = 50
    ad_ids = [f"{tenant_slug}_ig_ad_{i}" for i in range(n_ads)]
    ...
    ads_df = pl.DataFrame({
        ...
        "name": [f"IG Creative: {fake.bs()}" for _ in range(n_ads)],
        "creative_id": [f"ig_cr_{fake.uuid4()[:8]}" for _ in range(n_ads)],
        "status": "ACTIVE"
    })
    ...
    impressions = (spend * np.random.uniform(10, 50)).astype(int) 
    clicks = (impressions * np.random.uniform(0.005, 0.03)).astype(int) 
    conversions = (clicks * np.random.uniform(0.05, 0.20)).astype(int) 
