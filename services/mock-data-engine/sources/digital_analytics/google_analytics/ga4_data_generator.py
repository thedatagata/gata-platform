...
def generate_ga4_data(tenant_slug: str, config: Any, shopify_orders: List[dict], ad_campaigns: List[dict]) -> Dict[str, List[dict]]:
    ...
    for order in shopify_orders:
        ...
        is_paid = random.random() > config.organic_traffic_ratio
        ...
        events.append({
            ...
            "event_params": [{"key": "page_location", "value": landing_url}, {"key": "ga_session_id", "value": random.randint(100000,999999)}]
        })
        ...
        events.append({
            ...
            "event_params": [{"key": "page_location", "value": f"{landing_url}/thank_you"}, {"key": "ga_session_id", "value": random.randint(100000,999999)}]
        })

    # --- 2. Generate 'Noise' Traffic (Non-converting sessions) ---
    # Formula: Total Sessions = Orders / Conversion Rate
    # If 100 orders and 5% CVR, we need 2000 total sessions (1900 noise)
    if config.conversion_rate > 0:
        total_sessions_needed = int(len(shopify_orders) / config.conversion_rate)
    ...
    for _ in range(noise_sessions):
        # Time distribution (random over the period)
        ...
        events.append({
            ...
            "traffic_source": source,
            "geo": {"country": "United States", "city": fake.city()},
            "event_params": [{"key": "ga_session_id", "value": random.randint(100000,999999)}]
        })
